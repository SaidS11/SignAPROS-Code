...
import { setMongoInsertObject, setViewObject } from 'redux/slices/SeñalesSlice';
import {
  setFailUpload,
  setIsLoading,
  setIsUploaded,
...
import VerSignal from './VerSignal';
...
const VerSignalContainer = () => {
  ...
  const objetoMongo = useCustomSelector(
    (state) => state.señales.viewObject
  );
  ...
  const onClickCrear = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (dataR.etiqueta === '') {
      ...
      objCopy.name = "Martha Lopez";
      objCopy.protocol = "PRUEBA";
...
export default VerSignalContainer;
